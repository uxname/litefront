...
| NAM-05 | Magic numbers и magic strings заменены именованными константами | MINOR | 4/5 | 1) `ErrorFallback.tsx:179` — `setTimeout(() => ..., 2000)` — магическое число `2000`. 2) `env.ts:24` — `PORT: import.meta.env.PORT \|\| "3000"` — магическая строка `"3000"`. | 1) Вынести в `COPY_FEEDBACK_DURATION_MS = 2000`. 2) Вынести в `DEFAULT_PORT = "3000"` или читать из конфига. | Нет |

...
const stubUpload = (path: string) => {
  const fetchMock = vi.fn().mockResolvedValue({
    ok: true,
    json: async () => [
      { filename: "avatar.png", key: "2026/09/05/12-30/9f1c.png", path },
    ],
  });
...
describe("uploadAvatar", () => {
  // The backend stores uploads in S3-compatible storage and answers with a link
  // to the object — signed and expiring unless files are public — so the
  // frontend must pass it through untouched, signature included. Prefixing the
  // API origin would produce "https://api.test/http://storage.test:3900/…" — a
  // dead link; dropping the query would produce one the storage refuses.
  it("C6: returns the signed storage URL from the backend unchanged", async () => {
    const signed =
      "http://storage.test:3900/uploads/2026/09/05/12-30/9f1c.png" +
      "?X-Amz-Expires=900&X-Amz-Signature=deadbeef";
    const fetchMock = stubUpload(signed);

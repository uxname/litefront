export default {
  ...
  // You may also specify an array of patterns: the keys from the catalog that
  // are no long in the code but match one of the patterns will be kept.
  ...
  // That way, `t('Status: Loading...')` will not think that there are a namespace
  // and three separator dots for instance.
  ...
  // If you want to use plain english keys, separators such as `_` might
  // conflict. You might want to set `pluralSeparator` to a different string
  // that does not occur in your keys.
  // If you don't want to generate keys for plurals (for example, in case
  // you are using ICU format), set `pluralSeparator: false`.

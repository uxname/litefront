...
| F3 | AuthGuard — dead code (экспортируется, но не используется) | `src/features/auth/ui/AuthGuard.tsx` + `src/features/auth/index.ts:10` | LOW | Компонент экспортируется в public API, но не импортируется ни одним роутом. Защита роутов реализована через `beforeLoad`. Может запутать разработчиков. | ✅ AuthGuard удалён |

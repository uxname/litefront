...
| SEC-04 | .env.example содержит только placeholder-значения | ❌ FAIL | Medium | `.env.example:7-8` — `VITE_OIDC_AUTHORITY=https://oalmxx.logto.app/oidc` и `VITE_OIDC_CLIENT_ID=foxpqpb09d3rzxn9r4pfj` выглядят как **реальные dev-credentials** (Logto tenant + client ID). Должны быть placeholder-значениями. | **A:** Заменить на `https://your-oidc-provider.example.com`. **B:** Заменить на `your-client-id`. **C:** Убедиться, что эти credentials не отозваны, если настоящие. | ✅ Заменены на placeholder |
